...
fn split_scenario(fs: &mut Fs<TornBadBlock>) {
    let mut a = buf();
    let mut b = buf();
...
fn split_relocation_atomic_across_every_power_loss() {
    ...
    for trigger in 1..=total {
        ...
        assert_dir_consistent(&mut fs, &mut a, &mut b, trigger);
...
fn assert_dir_consistent(fs: &mut Fs<TornBadBlock>, a: &mut [u8], b: &mut [u8], trigger: usize) {
    ...
    fs.list_dir(Path::new("/d").unwrap(), |e| names.push(e.name.to_vec()), a, b)
        .unwrap_or_else(|e| panic!("list_dir after tear@{trigger} failed: {e:?}"));
    let mut sorted = names.clone();
    sorted.sort();
    sorted.dedup();
    assert_eq!(sorted.len(), names.len(), "no duplicate entries after tear@{trigger}");
    for name in &names {
        ...
        let got = fs
            ...
            .unwrap_or_else(|e| panic!("read {path} after tear@{trigger} failed: {e:?}"));
        if name.starts_with(b"zz") {
            continue;
        }
        assert_eq!(got, BIG.len(), "entry {path} length after tear@{trigger}");
        assert_eq!(out, BIG, "entry {path} payload after tear@{trigger}");
    }
}
