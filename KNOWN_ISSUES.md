...
- [ ] Wear leveling via pair relocation (v1.2). Within-pair wear distribution happens naturally via compaction-on-fill, which alternates active and alternate blocks of each pair. The remaining gap is **inter-pair** wear: a workload that updates one subdir's entries repeatedly wears that pair's two blocks while the rest of the device sits idle. The C-reference fix is to relocate a pair to fresh blocks every `BLOCK_CYCLES` compactions, updating the parent's DirStruct/HardTail body to point at the new blocks. **Design sketch**: compact-time predicate `should_relocate(pair, root, rev, BLOCK_CYCLES)` triggers redirection from the alternate-in-place compact to a fresh-pair compact. A new `WriteOp::UpdateDirStruct { id, new_pair }` lets the parent's commit flip the reference atomically. `find_parent(storage, root, target_pair, ...)` BFS-walks from root to locate the parent and id. Recursive: a parent-update commit might itself trigger relocation up the tree. Edge case: root pair never relocates (no parent; lives at fixed blocks 0/1). Deferred because the design is well-understood but the implementation is a multi-hour focused effort, and SMIL's CTZ audit-log workload spreads wear naturally through chain extension rather than directory updates.
