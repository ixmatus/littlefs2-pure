...
- [x] Wear leveling via pair relocation (v1.2). Within-pair wear distribution happens naturally via compaction-on-fill, which alternates active and alternate blocks of each pair. **Inter-pair** wear is now addressed by compact-time relocation: when the predicate `should_relocate(pair, root, new_rev, BLOCK_CYCLES)` fires (matching the C reference's `(rev + 1) % ((BLOCK_CYCLES + 1) | 1) == 0` modulus), the compact bytes are programmed both to the existing alternate (for durability) and to a freshly allocated block; the parent's `DirStruct` entry then flips to the new pair address via a new `WriteOp::UpdateDirStruct { id, new_pair }` op. `find_parent_in_tree` BFS-walks from root through `DirStruct` references to locate the parent. The parent's own commit dispatches through the same compact-or-relocate machinery, so a relocation can recurse up the tree until it lands inline at some ancestor. The root pair never relocates (its DirStruct has no parent; the spec pins root at `(0, 1)`). `BLOCK_CYCLES <= 0` disables the predicate. (`src/fs.rs::compact_and_program`, `src/fs.rs::propagate_relocation`, `src/alloc.rs::scan_used_with_single_buf`, `tests/wear_leveling.rs` covers root-stays-fixed, subdir-relocates-after-N-cycles, durable-across-remount, and the recursive parent case.)
