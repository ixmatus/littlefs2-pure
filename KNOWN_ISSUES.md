...
- [x] Failure driven relocation past a worn block, for metadata pairs as well as CTZ data (`lfs-23f`). A metadata commit that cannot write one of the pair's blocks relocates the pair onto a freshly allocated block instead of returning `Error::Io`. The compacted commit lands only on the fresh block (it is never written to a pair block, so there is no in place anchor as in wear levelling); the parent's `DirStruct` repoint is the sole linearization point, and the reachable `RelocateState` aggregate stays balanced (zero before the repoint, a cancelling pair after it), so the mount-time relocation recovery never fires on the worn pair it could not write. Covers a worn alternate during a plain compaction or a directory split, a worn continuation block (excluded and reallocated), and a worn active block hit by an append (the append falls back to a relocating compaction that eagerly evicts the worn block). The root pair cannot relocate, so a worn root commit stays `Error::Io`; retries are bounded by `MAX_BAD_BLOCK_RETRIES`. The CTZ data and append paths landed earlier (Part 1). Verified by `tests/pending_badblock_reloc.rs`, `tests/badblock_split_reloc.rs`, and `tests/badblock_reloc_crash.rs` (bounds, gstate balance, and a power-loss sweep). Design in `docs/decisions/0014-failure-driven-pair-relocation.md`.
...
- **HardTail-chain directory splitting and continuation-pair relocation.** Was a v1.x non-goal (the v1.0 writer never emitted `HardTail` tags, so directories capped at one metadata pair). **Implemented in the 2.x line** (ADR-0013, review item `lfs-cvh`): the writer now splits directories across `HardTail` continuation pairs — subdirectories and the root — threads them into the global list, relocates continuation pairs under wear-levelling, and is crash-safe and bidirectionally interoperable with the C reference (`roundtrip_split_dir`, `roundtrip_split_root`). No longer out of scope.
- **Reachable metadata-pair set larger than `MAX_QUEUED_PAIRS = 32`.** `Fs::mount` runs the `accumulate_gstate` move/relocation-recovery sweep, a deduplicating BFS over the directory forest bounded by `MAX_QUEUED_PAIRS = 32` (`src/alloc.rs`). An image whose reachable pair set exceeds 32, including a single directory split across more than 32 continuation pairs, is rejected at mount with `Error::OutOfRange` before enumeration is reached. Now that this crate's own writer produces split directories (ADR-0013), it refuses *at write time* to grow the reachable forest past the budget — the split and `mkdir` paths run a `collect_live_tree_pairs` pre-check (review items `lfs-cvh.4`/`.5`, `lfs-43o`) and degrade or `OutOfRange` rather than emit an image they could not remount. The tail-walk layer itself is cycle-safe and cap-free (Brent's, ADR-0009); lifting the end-to-end limit would require an unbounded dedup set in a no-alloc kernel and would enlarge the ADR-0006-pinned Cortex-M0+ stack arrays, so it remains a deliberate constraint, not a defect. Documented in ADR-0009.
...
- **Residual C gstate (orphan count, needssuperblock) is not cleared.** The C reference packs an orphan count, a needssuperblock flag, and a summary bit into the gstate tag word alongside the pending move; this crate models only the move. C strips the count and the superblock bit before committing (`lfs.c:2024`, `lfs.c:2275`), so the only pattern a C image carries is the summary bit `0x8000_0000`, which C reads back as one orphan (`lfs.c:4556`). Under this crate that word makes `has_pending_move` report true while `pending_move` yields nothing, so mount fires no recovery and the residue persists until a C mount runs its deorphan pass. Mount, reads, writes, and move recovery are unaffected, and the orphan bit cannot mask a genuine in-flight move; all of it pinned by `tests/review_l5_gstate_orphan_bits.rs` (review L5). Resolution ships with the `Fs`-resident gstate aggregate; see ADR-0016's out-of-scope section.
- **A compacting split cuts at most `MAX_SPLIT_CUTS` (8) times.** The bound is `log2(MAX_LIVE_ENTRIES)`, the depth the halving cut needs in the expected case; a size distribution needing more degrades to the single-block-commit fallback and may return `OutOfRange`, never corruption. No reachable case exceeding the bound has been found (review L1).
- **The geometry gate is a post-monomorphization const error.** `cargo build` and `cargo test` report a non-conforming `Storage` geometry as a compile error naming the violated relation; `cargo check` and `cargo clippy` do not evaluate the constant and stay silent. Downstream users who want the error at check speed add `const _: () = littlefs2_pure::geometry::Geometry::<MyFlash>::CHECK;` beside their impl. A type-level form visible to `cargo check` changes the `mount` and `format` signatures and is deferred to 2.0 (ADR-0019).
