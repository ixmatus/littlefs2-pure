...
The P3 tail: the five follow-ups the v1.3.0 arc filed against itself, closing the tracker to zero open items.

- **The out-of-range pair posture is one rule at every fetch** (`lfs-w3o`, closing the residue v1.3.0 shipped pinned). The HardTail chase sites (`resolve`, `find_dir_pair`, `seek_entry_in_chain`, `list_pair_chain`, `deorphan_sweep`, `unthread_and_steal` and their kin) handed decoded addresses straight to `Storage::read` and reported an out-of-range one as `Io`, misreading image corruption as a hardware fault. All sixteen pair-block reads in `src/fs.rs` now route through one bounds-checked helper that rejects out-of-range as `Corrupt` before the read; a genuine device rejection still reports `Io`. This is where the C reference puts the check (`lfs_dir_fetchmatch`, `lfs.c:1103`), so every fetch is covered by construction; the residue's pinning test is flipped to the new answer. Error-path behavior change: a caller matching `Io` to retry hardware faults no longer retries corruption.
- **CTZ block addresses get the same discipline on the allocator walk** (`lfs-0ph`). The H8 guard bounds a chain's total block count, but an individual skip pointer naming block 200 on an 8-block device passed it, was recorded in the tracking bitmap as a phantom, and was then dereferenced as `Io` (or refused as `OutOfRange` past the bitmap). `alloc::walk_ctz_chain` now checks every address before marking or reading, matching the read path, and `Fs::stream_ctz_extend` checks the chain head it dereferences directly.
- **`OpenOptions` setters commute** (`lfs-f4m`). `append(true)` set the `write` field as a side effect while `write(on)` overwrote it unconditionally, so `.append(true).write(false)` and `.write(false).append(true)` built different values. The flags are now independent and write access is the derived `write || append`, read at open time, matching `std`. Behavior change, widening only: `.append(true).write(false)` is now a writable append-mode open where it was previously rejected; the L6 rules (create and truncate need write or append) are unchanged and re-verified.
- **`shrink_ctz_head` retries its partial-tail relocation past worn blocks** (`lfs-i59`), the last fresh-block path outside the ADR-0014 discipline. An erase failure, a program failure, or a read-back mismatch on the fresh candidate excludes it and allocates another under `MAX_BAD_BLOCK_RETRIES` instead of failing the whole `set_len`. Only the fresh block is ever written, so the retry is purely additive and an abandoned candidate is an unreferenced orphan; the retry count is pinned exactly, not floored.
- **Read-back verification reads the device, not the alignment adapter's cache** (`lfs-6ym`; ADR-0020). `NorAlignedStorage` splices its pending program window into reads, so every verify site's last `PROG_SIZE` window was compared against RAM, and `sync` did not close the hole because the window stays resident and keeps splicing. Behind the adapter one worn program page produced a `write_to_path` returning `Ok` over altered user data, a `mkdir` returning `Ok` on a pair that read `Corrupt` at the next mount, and an `append_to_path` returning `Ok` where the design specifies `Io`. `Storage` gains a defaulted `read_device` method ("what does the device hold"), used by the two verify helpers and overridden by the adapter to flush an overlapping window and read through, mirroring the C reference's validating compare, which bypasses both its caches. Additive under semver; device op counts measured identical before and after. Wrapper storages must forward `read_device` or the blind spot returns at their layer; the obligation is documented on the method and in `INTEGRATION.md`.

## [1.3.0] - 2026-07-28

### Added

The P2/P3 backlog arc (2026-07): every remaining open item from the 2026-06 deep adversarial review, the coverage debt V4-V6, and the references-program followups, driven as a supervised multi-agent pass with every finding reproduced before it was fixed. With this arc the 2026-06 review is fully dispositioned: every Critical, High, Medium, and Low is fixed, refuted with a recorded argument, or deferred behind a pinned safety property.

- **Torn sweeps at device program granularity with partial-program landings** (review coverage V4, `lfs-hki`). The highest-value power-loss sweeps (inline write, CTZ streaming append, directory split, rmdir unthread, bad-block split relocation) now also run with the tear injector inside `NorAlignedStorage` over a strict NOR backing, so tears land at real `PROG_SIZE` window boundaries rather than kernel program-call boundaries, and an interrupted program can land only a prefix of its window (sampled at offsets 0, 1, half, and last; the sampling bound is documented on the constant). A witness assertion fails the sweep if no partial landing ever changed the device image, so the coverage cannot rot into vacuity. All sweeps pass; the overclaims the 2026-06 review found were test-side and stay fixed.
- **A second (and third) test geometry** (review coverage V5, `lfs-4s3`). The storage doubles are const-generic over geometry with aliases keeping every existing test unchanged; `tests/geometry2.rs` re-drives the geometry-sensitive scenarios (CTZ capacities and off-grid reads, the inline boundary, splits including a re-derived multi-cut, wear rotation, torn sweeps at both granularities) at 512/32, `tests/geometry_4k.rs` covers 4096/256, and `tests/geometry_ratios.rs` walks BLOCK_SIZE/PROG_SIZE ratios 4 through 32, the axis all three principal geometries share at 16. The kernel passed everywhere; the only corrections the work produced were to test-model assumptions, now documented in the test files.
- **Kani harnesses for CTZ geometry totality and the commit writer** (review coverage V6, `lfs-7zj`). `ctz_proofs` proves `skip_pointers_in_block`, `content_bytes_in_block`, `block_index_at_offset`, and `block_count` total on fully symbolic `u32` inputs at every block size meeting the C reference's 128-byte floor (`lfs.c:4189`), with a `should_panic` harness proving that floor tight rather than conservative; `commit_writer_proofs` proves `meta::Commit` never writes outside the caller's buffer, always terminates in a well-formed CCRC tail, has exact bounds checks that leave the cursor untouched on rejection, and refuses a caller-supplied CCRC. Nine harnesses, all discharged; the writer-to-reader round trip defeated CBMC and the module doc records the measurements and names the stacks that cover it instead. Discovered and filed while proving the floor: nothing in the crate enforces it (`lfs-cw1`).
- **Reverse-coverage guards for the spec's closed sets** (`lfs-432`). `tests/closed_sets.rs` parses the vendored `lfs.h` at test time and demands a recorded classification for every upstream tag type and error constant, pins a per-bucket census of all 2048 tag type values (never an aggregate, so compensating drifts cannot hide), pins `#[non_exhaustive]` on the public enums, and records a stated decision for every upstream error code the crate maps differently. The census promptly earned its keep during integration by catching the CommitCrc widening below.
- **CI integrity checks for the reference registry** (`lfs-lrs`). `tools/check_references.sh` verifies every recorded sha256 in `docs/references/` on every push (a vendored artifact with no recorded hash fails; living internal documents with `provenance: internal` and no hash are exempt by design); `tools/probe_reference_links.sh` probes canonical and archived URLs in a weekly scheduled workflow so network flakes cannot break push CI. All 18 recorded hashes verified against the current registry.
- **The `property_ctz` oracle is independent of the implementation** (review L8, `lfs-9z5`). The old oracle called the crate's own capacity functions, so a shared misconception passed; the new one derives pointer counts by literal divisor counting from the pinned `DESIGN.md` rule, lays files out by accumulation, and adds hand-computed vector tables at three block sizes. Both the mandated off-by-one mutation and the shared-misconception mutation (block 0 miscounted consistently on both sides) now fail the suite; the old suite provably passed the latter.
- **The C orphan-bit gstate divergence is pinned, documented, and deferred** (review L5, `lfs-1nh`). The C reference strips the orphan count and needssuperblock bits before committing (`lfs.c:2024`, `lfs.c:2275`), so the only pattern a C image carries is the summary bit `0x8000_0000`; under this reader that decodes as a pending move with no decodable move, mount fires nothing, and `tests/review_l5_gstate_orphan_bits.rs` pins mount success, byte-stability across consecutive mounts, readable entries, working writes, and that the orphan bit cannot mask a genuine in-flight move. Full modeling ships with the `Fs`-resident gstate aggregate (ADR-0016 out-of-scope note).

- **C-written conformance vectors for the image classes that hid the top findings** (review coverage V1, `lfs-761`). Five golden images generated by the pinned C oracle now sit beside the baseline seven at `tests/vectors/`, each pinning the read direction of a fix against a genuine C image rather than a hand-built one: `08_user_attrs` (three entries with user attributes, the middle removed so the survivors' live ids shift under the delete, exercising splice-aware attribute reads for C1/C2), `09_deep_ctz` (a 900-byte, four-block CTZ chain that walks multi-level skip pointers past what `03`/`06` reach), `10_delete_tombstone` (a bare delete tombstone beside a live neighbor, the C-writes/Rust-reads companion to the roundtrip `remove` scenario for C3), `11_compacted_rename` (a directory the oracle itself compacted after a rename, whose NAME tags are non-monotonic in log order — ids `1,2,4,3` — the exact shape a reader requiring id-dense NAME order rejected before the H1 fix), and `12_multimove_gstate` (two cross-directory renames into one directory leaving two `MOVESTATE` tags in one log, which an XOR-accumulating reader decodes as a phantom pending move; C4). Each has a `tests/conformance.rs` test and a CRC32 pin, so an accidental regeneration that changes any byte fails the suite until the pin is updated in the same commit. This retires the hand-built H1 placeholder's `lfs-761` follow-up note.
- **The roundtrip gate is now read-write** (review coverage V2 / M11, `lfs-jdk`). `tools/verify_image` gained a `mutate` scenario: it mounts a Rust-formatted image, writes an inline file and a CTZ file into it (forcing C to append a commit into the erased region Rust left an FCRC over, and to allocate data blocks inside a Rust device), and dumps the mutated image back for Rust to remount and verify. `tests/roundtrip.rs::roundtrip_c_writes_into_rust_image` drives it end to end. This proves the FCRC / erased-window handshake in the C-writes-into-Rust direction, the one direction the previously read-only gate never exercised. Deeper interleaved-writer histories remain follow-up `lfs-4b8`.

### Fixed

The P2/P3 backlog arc (2026-07), continued: the bug half.

- **The CTZ read path issues only `READ_SIZE`-aligned reads** (`lfs-8e6`, the read-path twin of M7). `ctz::collect_chain_blocks`, `ctz::seek_block`, and every content span in `ctz::read_ctz_at` issued reads off the alignment grid in offset or length, violating the `Storage` precondition and faulting on hardware that enforces the grid; RAM-backed doubles tolerated them, so no suite caught it. All three now stage through `storage::read_range`, a new aligned read-through helper pinned by an exhaustive extent sweep, at an unchanged device read count (measured: 51 reads before and after for a 32-block file). New `ctz::collect_chain_blocks_buffered` / `ctz::seek_block_buffered` serve callers holding a block buffer; the bufferless forms keep their signatures up to `MAX_UNBUFFERED_READ_SIZE` (512).
- **File-data programs are read back and verified** (`lfs-ttr`), extending review H2's discipline from the eight metadata commit sites to the five file-data sites. The initial CTZ chain write, the copy-on-write tail rebuild, and the streaming append's overflow blocks relocate past a lying block exactly as they relocate past one that reports failure; the committed tail fill and the shrink relocation report `Io` with acknowledged data intact (each non-relocating verdict carries its reason at the call site). Discovered and filed alongside: the NOR adapter can satisfy a read-back from its dirty cache (`lfs-6ym`) and `shrink_ctz_head` lacks the retry loop (`lfs-i59`).
- **`mkdir` retries its pair init commit past worn blocks** (`lfs-n23`) under `MAX_BAD_BLOCK_RETRIES` with per-block exclusion, the ADR-0014 discipline every other fresh-block path already followed; the abandoned candidate stays an unreferenced orphan the allocator reclaims. The retry count is pinned exactly, not floored.
- **A compacting directory split cuts as many times as it takes** (review L1, `lfs-i4y`). The split point bounds only the upper portion, so an operation growing an entry already in the lower portion (`set_attr`, an inline rewrite, a longer rename) could leave a remainder no block holds, failing `OutOfRange` where the C reference splits again (proven with a C driver running the identical sequence). The writer now re-measures after every cut, bounded by `MAX_SPLIT_CUTS`, the reachable-pair budget, and the root fullness guard, with the retry budget shared across cuts; every continuation stays unreferenced until the single lower-half commit linearizes the chain, pinned by a torn sweep across all three program boundaries and a C-reads-Rust `multi_cut` roundtrip scenario. The finding's second half (forced-victim relocation paths never split) is refuted for this writer's own images: a 183-case sweep over the worn-active-block branch found zero failures, and ADR-0013's revision records the real arithmetic margin plus the C-authored-image caveat that could one day consume it.
- **Commit CRC recognition follows the C reader's acceptance rule** (review L2, `lfs-bdv`). `lfs_dir_fetchmatch` ends a commit on `lfs_tag_type2`, which masks the type field with `0x780`, so every `0x5xx` chunk `0x00..=0x7f` terminates a commit; this crate recognized only `0x00..=0x03`, silently truncating every commit from the first unrecognized terminator onward. Verified with the C verifier mounting and reading the exact image pre-fix Rust truncated. `meta::Commit::tag` now also refuses the whole widened family as data tags, and the closed-set census pins the classifier at 128 of 2048 values.
- **Corrupt splice streams surface as `Corrupt`, not `NotFound`** (review L4, `lfs-ceq`, per the 2026-06-11 adjudication). `dir::lookup` keeps its frozen `Option` signature as a thin wrapper; a crate-internal checked variant carries the distinction and every `Fs` resolve and write path uses it, so corruption no longer invites a create into a damaged pair. Mount's whole-tree sweep already refused such images, so this is defense in depth; the shadow is pinned by a test so a future lazier mount surfaces it.
- **A read-only open can no longer mutate flash** (review L6, `lfs-c5b`, per the adjudication). `create(true)` with an access mode granting no write materialized the missing entry anyway, programming the parent pair and leaving a phantom entry that poisoned later opens of the same name. `Fs::open` now rejects `create` or `truncate` without write as `InvalidPath` before touching the device, matching `std::fs::OpenOptions`; tests pin zero programs, zero erases, unchanged image bytes.
- **Pair identity is the unordered physical block set at every membership site** (review L7, `lfs-a8j`). The gstate accumulation, live-tree collection, parent search, deorphan sweep, and both allocator walkers deduped or matched on the ordered tuple, so an image naming one pair under both orders folded its gstate twice (XOR self-cancel, hiding a pending move) and the deorphan sweep could classify a live directory as an orphan and drop it. The C reference is order-insensitive by construction (`lfs_pair_issync`, and its deorphan pass treats swapped-order pointers as healthy); no C trace producing the alias was found, so this lands as robustness against corrupt or adversarial images.
- **The allocator's forest walkers bounds-check every decoded pair** (review L9, `lfs-b2m`), the rejection the `Storage` docs promised and M3 delivered for gstate. One address range surfaced as `Io` (a device fault) and another was silently swallowed by the tracking bitmap's saturating reads; both now reject as `Corrupt` at every enqueue site, matching `lfs_dir_fetchmatch`. Filed from the audit: the C reference can write an explicit null-sentinel tail this crate's mount rejects (`lfs-yl6`), and the skip-versus-reject posture across walkers wants one documented decision (`lfs-qeh`).
- **`cargo doc` under `-D warnings` builds again**, fixing the CI doc job red since the review-highs merge: two public doc comments linked private items (`RENAME_ATTR_STAGE`, and the new checked lookup). The doc gate is now part of the arc's own definition of done.
- **Mount accepts C-written images carrying an explicit null-sentinel tail body** (`lfs-yl6`, found during the L9 audit). `lfs_dir_drop` commits the tail tag with a literal all-ones body when the dropped directory was last in the global thread, while compaction omits the tag; both spellings are conforming and the C reader collapses them via `lfs_pair_isnull` (an OR over the halves, mirrored exactly by the new `BlockPair::is_null`). This crate reported the sentinel verbatim and mount rejected the image as `Corrupt`. The collapse now happens at the tail decode chokepoint, after latest-tag-wins so a superseded link cannot resurrect, plus the raw-tag allocator walker; every consumer sees thread end in one shape. Reproduced by a new C-generated conformance vector (`13_null_tail`, the rmdir-of-last-directory history) that failed pre-fix; the twelve existing vectors regenerate byte-identically, and this crate's own tag-absent rmdir spelling is pinned as interoperable in both directions. With it, the out-of-range posture is unified (`lfs-qeh`): every walker rejects a genuine out-of-range pair as `Corrupt`, the rule stated once on `pair_in_bounds`; the HardTail chase sites still surface out-of-range as `Io` and that residue is pinned visibly and filed (`lfs-w3o`).
- **The geometry preconditions are enforced, at compile time where the compiler can carry them** (`lfs-cw1`, review design observation D9; ADR-0019). The 128-byte block-size floor the C reference asserts at mount (`lfs.c:4189`) was documented and Kani-proven tight but checked nowhere: a 64-byte device formatted and mounted, wrote an image the C reference refuses, and `content_bytes_in_block` aborted in debug while wrapping to near `u32::MAX` in release. `Fs::mount` and `Fs::format` now name `geometry::Geometry::<S>::CHECK`, so a non-conforming geometry is a build error naming the violated relation (post-monomorphization: `cargo build` and `cargo test` see it, `cargo check` does not, documented with the downstream recipe); `geometry::validate` is the folded-away runtime backstop and `ctz::read_ctz_at` carries the floor guard for the unmounted path. Seven relations enforced, four advisory ones deliberately not, each argued in ADR-0019; two new Kani harnesses prove the gate admits nothing below the floor and that every admitted geometry makes the CTZ capacity arithmetic total, discharging the assumption `ctz_proofs` previously trusted.
- **The Kani harness inventory in `src/verify/mod.rs` matches what the harnesses prove** (review L3, `lfs-5vp`). The `commit_proofs` entry described another module's content and called landed work pending; it now records the nondeterministic CRC stub's actual meaning (panic-freedom strengthened, accept-or-reject correctness explicitly NOT proven here but by the conformance and roundtrip vectors). Three more stale or incomplete entries corrected against the harness code; review R4 fixed the same overstatement in `KNOWN_ISSUES.md` and missed this copy.

The Medium bug findings from the 2026-06 deep adversarial review (M1-M11; with these every Critical, High, and Medium bug from that review is closed).

- **rmdir drops the whole directory chain, not just the head** (review M1, `lfs-tx8`). Un-threading a removed directory re-pointed the thread predecessor at the dropped head pair's tail; for a HardTail-split directory that tail is a HardTail into the directory's own continuation, so the predecessor (in practice the root pair) inherited a HardTail into the removed directory's leftover pairs, grafting them onto an unrelated chain. Emptying a split directory does not collapse it, so this was reachable from the crate's own operations. `unthread_and_steal` now walks the entire HardTail chain, stealing every pair's gstate, and re-points the predecessor at the last pair's SoftTail thread link past the whole directory. Pinned by `tests/review_m1_rmdir_multipair.rs`.
- **Entry names are capped at `NAME_MAX` (255), not the tag length ceiling** (review M2, `lfs-ax2`). The raw-name write APIs validated against `0x3FF` (1023); a longer name produced an entry the C reference cannot reach. Both raw-name commit chokepoints now reject a name over `NAME_MAX` with `InvalidPath`. Pinned by `tests/review_name_max.rs`.
- **Mount recovery bounds-checks gstate-decoded pair addresses** (review M3, `lfs-fr8`). A corrupt or adversarial `MoveState`/`RelocateState` body naming an out-of-range pair drove mount recovery to dereference (and would write through) an out-of-bounds block; the decoded pairs are now `pair_in_bounds`-validated before recovery touches them, failing the mount with `Corrupt` instead. Pinned by `tests/review_m3_gstate_bounds.rs`.
- **The roundtrip gate fails, not skips, when the verifier is missing under CI** (review M4/V7, `lfs-4d4`/`lfs-wee`). `require_verifier` forks on `LFS_REQUIRE_VERIFIER`, which the CI roundtrip job sets, so a missing verifier binary is a hard failure there rather than a silent skipped-to-green run.
- **The allocator's CTZ chain walk issues `READ_SIZE`-aligned reads** (review M7, `lfs-mqz`). `walk_ctz_chain` read a skip-pointer header with a 4/8-byte read, violating the `Storage` alignment precondition; it now fetches a `READ_SIZE`-aligned window through a block buffer. Pinned by `tests/review_m7_read_alignment.rs`. (Discovered while pinning it: the CTZ read path has the same issue, filed as `lfs-8e6`.)
- **A failed `NorAlignedStorage` flush no longer poisons later flushes** (review M8, `lfs-ru2`). `flush` left the window dirty after a program failure, so every later flush retried the same failing program: one worn block blocked all subsequent writes. `flush` now clears and invalidates the window before propagating the error; the kernel relocates the data. Pinned by `tests/review_m8_nor_flush.rs`.
- **The CTZ publish carries its chain as coordinates, not a block list** (review M9, `lfs-yka`; hardening). Materializing the in-flight chain as a block list capped the honored exclusion at the commit-internal `2 + MAX_QUEUED_PAIRS` arrays; the publish now carries `(head, size)` coordinates walked on demand, uniform with the streaming-append path, so no exclusion array can overflow. The overflow was recoverable via the split degrade-to-compaction fallback and not reproducible end to end; the fix is applied as hardening. Coverage in `tests/review_m9_inflight_capacity.rs`.
- **The allocator BFS dedups enqueues against the pending queue** (review M10, `lfs-nvw`). A pair reachable as both a DirStruct child and a tail successor was collected by two parents before being marked used, double-enqueuing it into the bounded queue and spuriously overflowing it; both enqueue loops now skip a child already present in `queue[head..tail]`.
- **The read-write roundtrip gate proves the C-writes-into-Rust direction** (review M11, `lfs-4b8`; satisfied by the V2 `mutate` scenario above).

The five remaining Highs from the 2026-06 deep adversarial review (with these, every Critical and High from that review is closed). Design record for the two kernel families: ADR-0018.

- **Every commit is read back and CRC-verified after programming** (review H2, `lfs-efe`; ADR-0018). The C reference re-reads every commit it programs (`lfs_dir_commitcrc` via `lfs_bd_crc`) and treats a mismatch as a worn block; this crate trusted `program`'s `Ok`, so a device that lands corrupted cells silently (a real worn-NOR failure mode) got durable-success reports and surfaced the loss only at the next mount. All eight commit program sites now verify through one helper (`verify_programmed`, CRC compare against a read-back into the just-programmed buffer region) and a verify failure takes the same path as a program failure: the in-place append falls into worn-block eviction, the compact-to-alternate diverts to fresh-block relocation, the wear-relocation copy abandons and cancels, the bad-block retry excludes the candidate, and mkdir's pair init and `format`'s superblock report `Io`. Pinned by `tests/review_h2_readback.rs` with a silently-corrupting storage: data survives remount via relocation on a lying non-root block, and a lying root surfaces an error with prior state intact (all three failed pre-fix).
- **Cross-directory rename moves the entry's user attributes with it** (review H6, `lfs-inm`; ADR-0018; closes H5's `lfs-e7i` with the suite below). The destination commit rebuilt the moved entry from NAME and STRUCT alone, so every user attribute died silently on every cross-directory rename; the C reference's `LFS_FROM_MOVE` replays all unique tags of the moved id inside the destination commit. The rename now stages the source entry's live attributes (the splice-correct `for_each_live_attr` walk from ADR-0015) through a 1 KiB stack pool into the Create-family ops, and both emission paths (append `emit_op`, compaction `emit_compact_range`) re-emit them atomically with the create and its MoveState tag. One documented divergence: an entry whose live attribute payload exceeds the 1 KiB stage fails the rename with `OutOfRange` (nothing moved, attributes intact) where C, which streams through its block caches, would succeed. Pinned by `tests/attr_suite.rs` across all three struct kinds, the compacting-destination path, delete-marker non-resurrection, and remounts (five reproducers failed pre-fix); the suite also pins H5 (attributes on HardTail continuation entries, whose read half landed with ADR-0015).
- **The allocator's CTZ chain walk is bounded by the device** (review H8, `lfs-25d`). The read and write paths cap chain walks at `MAX_CTZ_BLOCKS` (a stack-array bound); the allocator scan streams and had no bound at all, so an adversarial `CtzStruct.size` drove `~size/block_size` skip-pointer reads on every rescan (about 16.8M reads for `size = u32::MAX` on 256-byte blocks, reproduced completing silently). The walk now rejects a chain claiming more blocks than the device holds as `Corrupt` before the first read. The bound is deliberately `BLOCK_COUNT`, not `MAX_CTZ_BLOCKS`: a C-written file larger than the read cap is unreadable by this crate but must still be marked in-use, or the allocator would hand out its blocks and destroy it on the next write (both directions pinned in `tests/alloc.rs`).
- **The torn-write sweeps now prove what they claimed** (review H7, `lfs-xzt`; closes M5 `lfs-1i3`, M6 `lfs-yzp`, V3 `lfs-e2a`, L10 `lfs-7r5`). The inline power-loss sweep accepted `Corrupt`/`Unformatted` remounts at every trigger, so "torn write bricks the filesystem" passed; the split and rmdir-unthread sweeps silently `continue`d past unmountable post-tear images; `pending_softtail_torn` discarded its own existence assertion with `let _`; and trigger ranges were armed over a count that excluded `format`'s program calls. One shared strong-semantics harness (`run_torn_scenario` / `mount_image_strict` / `torn_call_counts` in `tests/common`) now distinguishes the only legitimate mount-failure window (the tear landing inside `format` itself) from the bricked-device outcome, which fails loudly; the split sweep additionally asserts the surviving directory is an exact prefix of the write sequence, and both multi-pair sweeps assert remount stability. `TornWriteStorage`'s doc no longer claims NOR enforcement its inner storage does not provide (M6); running the sweeps through `NorAlignedStorage` with partial-program landings remains coverage item V4 (`lfs-hki`). All sweeps pass under the strict semantics: the overclaims were test-side, and the kernel invariants hold.

The final two Criticals from the 2026-06 deep adversarial review (with these, all nine review Criticals are closed):

- **Entry deletes carry length 0, the C reference's encoding** (review C3, `lfs-cna`). `emit_op` wrote entry deletes with the reserved length sentinel `0x3FF`; the C reference writes every entry delete with size 0, and `lfs_dir_fetchmatch`'s exact-compare besttag invalidation (lfs.c:1244) never matches the sentinel form, so a C mount resolved a Rust-deleted name to its NEIGHBOR entry, served the neighbor's content, and a C-side remove of the deleted name destroyed the neighbor. One-line writer fix; this crate's reader dispatches deletes on the id alone and accepts both encodings, so pre-fix images stay readable. Pinned bidirectionally: a new `remove` scenario in `tools/verify_image` (asserts `/aa` intact AND `lfs_stat("/bb")` returns `LFS_ERR_NOENT` under the C reference; failed pre-fix with the C binary reporting the deleted file present) plus a wire-level length assertion in `tests/remove.rs`.
- **Commit-internal allocations can no longer reallocate the in-flight CTZ chain being published** (review C9, `lfs-0vy`; design D7). `commit_update_ctz` (the `File::sync` and streaming-append publish path) entered the commit with an empty inflight set; a worn-block retry inside the commit clears the allocator cache and rescans, the rescan sees only committed blocks, and the lowest free blocks it hands the relocation are exactly the chain blocks the commit is publishing; erasing one destroys the file's data at the moment it becomes durable (reproduced deterministically: the file read back metadata bytes). The inflight parameter is now a required `Inflight` value carrying both named blocks and an un-committed chain as `(head, size)` coordinates; the chain is walked only on the rescan path, preserving ADR-0011's hot-path win, and `alloc_one_block_cached_single_buf` (all eight commit-internal allocation sites) honors it. Pinned by `tests/review_inflight_chain.rs`: a deterministic worn-set scenario steering the publish commit down the retry rescan, and a progressive wear-out churn over acknowledged publishes.

One more Critical from the 2026-06 deep adversarial review:

- **A failed or torn streaming append no longer poisons the committed CTZ tail block** (review C8, `lfs-ay4`; design record ADR-0017). `stream_ctz_extend` filled the committed tail's erased region before the overflow allocation and bounds checks; on a device-full failure or a power loss the metadata still said the old size while cells past the committed EOF were programmed, and the next append ANDed different bytes over them on NOR, corrupting acknowledged data (reproduced on strict NOR semantics: `0x99 & 0x55 = 0x11` read back as committed content). The append now runs every fallible step first and fills the committed tail last, and the fill region is verified actually erased: a torn previous fill routes the tail through copy-on-write to a fresh block (the `shrink_ctz_head` countermeasure; the C reference never programs a committed data block twice). The clean in-place fill stays the common case, preserving ADR-0011's write-amplification win. Pinned by `tests/review_ctz_append_poison.rs`: a device-full append followed by a small append, and a power-loss sweep at device program granularity (the torn wrapper sits inside `NorAlignedStorage`, so tears land at real program boundaries, not cache-flush boundaries).

Three findings from the 2026-06 deep adversarial review, the relocation-cascade family, plus a deeper latent defect their reproduction surfaced (design record: ADR-0016):

- **Cross-directory rename now survives a relocation cascade into its source pair** (review C6, `lfs-bkq`). The rename captured the source coordinates before the destination commit; that commit's relocation cascade could relocate the source, after which the source delete (and a crashed-rename mount recovery) targeted the orphaned old address: a permanent duplicate entry whose MoveState could never cancel. The `Fs` now carries the in-flight move's coordinates, `propagate_relocation` remaps them when the source relocates (verified load bearing: disabling the remap reproduces the duplicate at round 36 of the grid), the source commit folds the ORIGINAL delta at the remapped address, and `recover_pending_move` resolves a stale decoded source through its relocated twin (the unique live tree pair sharing a block as its inactive half). Pinned by a 60-round nested-rename grid under `BLOCK_CYCLES = 1` and a power-loss sweep over a relocating rename with consecutive-mount byte-stability (`tests/review_reloc_cascade.rs`).
- **An abandoned wear relocation cancels its own `RelocateState` immediately** (review H3, `lfs-gfm`). When the fresh block refused the program, the unbalanced body was left for mount-time recovery; a second relocation before remount outdated both addresses and recovery committed through a dead pair on every mount, forever. The abandonment now folds the cancelling delta back out of the pair's total at once, on the unchanged pair address. Pinned by a worn-fresh-block sweep.
- **The deorphan sweep repoints half-orphans instead of reclaiming them** (review H4, `lfs-les`). A crash between the relocation's parent and predecessor commits leaves the tree holding the relocated pair while the thread points at the outdated twin; reclaiming the stale link permanently dropped a live pair from the global thread the C reference's allocator and traverse walk. The sweep now re-points the thread at the tree's twin (the C reference's half-orphan pass: the tree is authoritative). Pinned by a power-loss sweep asserting the thread/tree sync invariant at every program boundary.
- **`find_parent_in_tree` consumes live state only** (latent, found reproducing C6; same family as C5). The parent BFS enqueued every raw `DirStruct` body, superseded ones included; once relocation reuses freed blocks, a stale pre-relocation pair's old log still matches the target, and the parent repoint then committed onto a DEAD pair (observed erasing and rewriting freed blocks) while the live parent kept the outdated reference, desyncing tree from thread and stranding gstate on unreachable pairs. The walk now enqueues splice-corrected latest-wins children and the latest tail only, and matches over live bodies, subsuming the C5 candidate discipline (the returned id is live by construction). This matches the discipline `accumulate_gstate` and `collect_live_tree_pairs` already followed, and the C reference's `lfs_fs_parent` by construction.

Two more findings from the 2026-06 deep adversarial review, the gstate family's first slice:

- **Per-pair gstate was decoded as XOR-of-all-tags; the C semantics is latest-total-wins** (review C4, `lfs-w7w`). The C reference's convention has two levels: within one pair's log every committed gstate tag is the pair's new TOTAL contribution (writers fold the existing contribution into each tag; the reader takes the single latest tag), and only across pairs does XOR accumulate. This crate read XOR-of-all-tags within a log, so a valid crash-free C image holding two MOVESTATE tags in one log (two moves into the same directory, no intervening compaction) decoded a phantom pending move, and mount recovery deleted a live entry (reproduced). Both halves changed together: `scan_pair_move_state`/`scan_pair_relocate_state` now take the latest tag (an explicit all-zero body is a real total and shadows earlier tags), and the append path folds the pair's existing total into every gstate tag it emits. Migration note: a v1.2.0-written image crashed mid-rename whose pair logs hold several unfolded delta tags decodes differently under the new reader; a clean (quiescent) v1.2.0 image is unaffected.
- **`rmdir` dropped a pair from the reachable set without stealing its gstate contribution** (review C7, `lfs-njj`). A completed rename out of a directory leaves that directory's pair with a non-zero total balanced globally by the destination's. Dropping the pair (rmdir's un-thread, or the mount-time deorphan reclaim) left the reachable aggregate permanently non-zero; every subsequent mount decoded a pending move against the dead pair, and once the dead pair's content no longer covered the decoded id the image failed to mount outright (reproduced: rmdir after a rename out made the filesystem unmountable). Both drop sites now route through `unthread_and_steal`, which folds the dropped pair's `MoveState` and `RelocateState` totals into the commit that re-threads the survivor, the C reference's `lfs_dir_drop` "steal state". Reproducers in `tests/review_gstate.rs`, including a consecutive-mount byte-stability check that fails on any futile recovery loop.

Four findings from the 2026-06 deep adversarial review (`docs/reviews/2026-06-10-deep-adversarial-review.md`), the splice and attribute family. All four shared one root cause: the live-id versus raw-tag-id mapping was re-derived independently at five sites and each copy missed part of the C reference's countermeasures. The structural remediation is ADR-0015: one forward splice core (`dir::splice_step`) feeds all four live-entry walkers, one backward splice-diff query (`dir::attr_get` over the new `meta::TagIterRev`) serves point reads, and one emission stream (`fs::emit_compact_range` over a `TagSink`) feeds both the compaction writer and the split-point size estimate so they cannot drift. Reproducers for every finding are pinned in `tests/review_splice_attrs.rs`; each failed against v1.2.0 before its fix.

- **Compaction destroyed all user attributes; `set_attr` on a full block returned `Ok` without persisting** (review C1, `lfs-2dg`). The compaction rebuild re-emitted each live entry as Create + NAME + STRUCT only, so every compaction stripped every attribute on every entry in the pair, and `SetAttr`/`RemoveAttr` matched a `_ => {}` wildcard, so a `set_attr` arriving when the active block was full compacted, persisted nothing, and reported success. The rebuild now replays each entry's live attributes (latest value per attr id, splice correct, delete markers consume their id) and merges the in-flight attr op, matching `lfs_dir_compact`. The op-to-effect mapping is an exhaustive match with no wildcard (review D4), so a future `WriteOp` variant fails to compile until its compaction effect is declared.
- **`get_attr` was not splice aware: attributes vanished across deletes and leaked across entries** (review C2, `lfs-3z8`). The read compared raw committed tag ids against the entry's current live id; deleting a lower-id entry made the attribute unreadable, and a new entry reusing the raw id read the previous entry's value. `get_attr` now queries through the backward splice-diff walk (`lfs_dir_getslice`'s `gdiff` algorithm) and chases HardTail continuations like `set_attr` already did, so attributes on split-directory entries are readable (review H5's read half).
- **`find_parent_in_tree` returned raw unspliced ids; wear relocation repointed the wrong parent entry** (review C5, `lfs-fb2`). Any Delete committed to the parent's log after the child's `DirStruct` tag made the raw id and the live id diverge; `propagate_relocation` then rewrote a sibling entry's struct body into a directory address (reproduced: a file's content replaced by a pair address) while the relocated child kept its stale parent reference. The parent walk now carries the C reference's candidate discipline (`lfs_dir_fetchmatch`'s `tempbesttag`): splices at or below the candidate shift it, an exact Delete or an overwritten `DirStruct` kills it, and the returned id is live by construction.
- **The reader rejected routine C-compacted directories** (review H1, `lfs-r88`). All four live-entry walkers required NAME tags id-dense in log order; C compaction emits surviving tags in log order, which after a rename is not ascending-id order and can place an entry's STRUCT before the NAME that establishes its count, so valid C images failed `Fs::mount` with `Error::Corrupt`. The shared splice core now grows the count to `max(id + 1, count)` on any NAME (the C reference's `tempcount` rule) and parks early STRUCT tags until their NAME claims the slot; splice tags keep their strict corruption checks. Pinned by a hand-built C-compaction-shaped image; the C-written conformance vector follows under `lfs-761`.
