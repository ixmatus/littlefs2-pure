...
- **Power loss during a wear-levelling relocation** — *recovered on next mount*. The compactor programs the new commit to the existing alternate *first* (the durability boundary), then copies it to a freshly allocated block, then commits the parent's flipped `DirStruct`. Each of those three programs carries a balanced `RelocateState` tag (16-byte body: `old_pair` LE pair + `new_pair` LE pair) so the contributions XOR to zero once all three land. A crash that lands fewer than three leaves a non-zero filesystem-global RelocateState aggregate; `Fs::mount` walks every reachable pair, XOR-accumulates every committed `RelocateState`, and if the result is non-zero decodes `(old_pair, new_pair)` and emits a balancing commit on `old_pair` that cancels the cycle. The forfeited fresh block becomes orphan and is reclaimed by the next allocator scan. The FS observes either the pre-relocation state (cycle cancelled) or the post-relocation state (cycle completed) — never a phantom intermediate where the parent points at a half-built pair.
