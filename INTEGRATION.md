...
| C-reference conformance | Four committed vectors generated by C littlefs (empty / inline / CTZ / nested-dir); `tests/conformance.rs` mounts each via our reader and asserts the expected entries |

The conformance harness covers the C-writes-Rust-reads direction. The reverse (Rust-writes-C-reads) requires a small C verifier companion and is tracked as a Phase 3 follow-up. With FCRC emission turned on for every `Fs` write path, the writer-side bit accuracy is now equivalent to what the C reference produces by construction; the remaining cross-mount test is what proves the equivalence end-to-end.
